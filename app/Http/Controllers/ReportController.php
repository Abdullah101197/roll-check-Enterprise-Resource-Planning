...
class ReportController extends Controller
{
    ...
    //             dd($data);
    //             // $employees2->push((object)[
    //             //     'name' => $employee->name,
    //             //     'monthly_salary' => $monthlySalary,
    //             //     'attendance_yearly' => $attendanceDays,
    //             //     'leaves_yearly' => $leaveDays,
    //             //     'holidays_yearly' => $holidaysCount,
    //             //     'loan' => $totalLoan,
    //             //     'allowance' => $totalAllowance,
    //             // ]);
                
    
    //             return view('report.employeeReport', compact('employeesAttendance', 'branch', 'department', 'employees', 'dates', 'data'));
    //         } else {
    //             return redirect()->back()->with('error', __('Permission denied.'));
    //         }
    ...
    public function employeeReport(Request $request)
{
    ...
    $employees = Employee::select('id', 'name', 'salary')->where('created_by', \Auth::user()->creatorId())->get();
    ...
    foreach ($employees as $employee) {
        $attendanceDays = AttendanceEmployee::where('employee_id', $employee->id)
            ->whereYear('date', $year)
            ->where('status', 'Present')
            ->count();

        $leaveData = Leave::where('employee_id', $employee->id)
            ->whereYear('start_date', $year)
            ->where('status', 'Approved')
            ->get();
        
        $leaveDays = $leaveData->sum('total_leave_days');
        $leaveReasons = $leaveData->pluck('leave_reason')->unique()->implode(', ');
        
        // Format leaves_yearly with both days and reasons
        $leavesYearly = $leaveDays . ' days (' . $leaveReasons . ')';

        $holidaysCount = Holiday::whereYear('start_date', $year)
            ->count();

            

        $totalLoan = Loan::where('employee_id', $employee->id)
            ->whereYear('start_date', $year)
            ->sum('amount');

        $totalAllowance = Allowance::where('employee_id', $employee->id)
    ...
    return view('report.employeeReport', compact('employees2'));
}
